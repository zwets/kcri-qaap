...
class FastQScreenShim:
    ...
    def execute(self, ident, blackboard, scheduler):
        ...
        execution = FastQScreenExecution(SERVICE, VERSION, ident, blackboard, scheduler)

         # Get the execution parameters from the blackboard
        try:
            fastqs = execution.get_all_user_fastqs().values() if Services(ident) == Services.FASTQSCREEN else \
                     execution.get_all_new_fastqs().values() if Services(ident) == Services.POST_FASTQSCREEN else \
                     None

            if fastqs is None: raise Exception('unknown ident in FastQScreenShim: %s' % ident.value)
