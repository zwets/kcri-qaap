#!/bin/sh
#
# Pins the versions in deps-versions.config to the current state of
...
NEW_FILE="$BASE_DIR/deps-versions.config.new"
...
Usage: $(basename "$0")
  ...
  Updates $(basename "$CFG_FILE") to reflect the current dependency versions,
  so that this release can be reproduced by running ./update-deps.sh.
...
grep -E '^ *[^#].*$' "$CFG_FILE" | while read NAME VER URL REST; do
    ...
    [ -d "$DIR" ] || err_exit "dependency directory not present: $NAME"
    ...
    if [ "${URL%gz}" = "${URL}" ]; then  # assume git
        ...
        CUR_VER="$(cat "$DIR/.qap_version_crumb" 2>/dev/null)" || 
            err_exit "dependency version unknown (not installed with update-deps?): $NAME"
    fi
